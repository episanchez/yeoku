 ...
 */
ComponentManager.prototype.create = function(obj, name){
	// new instance obj
	obj["id"] = this.idx;
	this.componentsType[name] = obj;
 ...
 */
ComponentManager.prototype.addComponentByName = function(entity, name, values){
	var ct = this.componentsType[name];

	this.componentsByType[ct.id][entity.getUID()] = entity.getUID();
	if (values)
		entity[name] = new ct(values);
	else {
		entity[name] = new ct();
	}
	if (this.world)
