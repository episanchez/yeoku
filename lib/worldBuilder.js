...
exp.BuildWithWorldConfiguration = function(worldConfiguration){
	...
	var optManagers = worldConfiguration.optManagers();
	...
	for (var i = 0; i < componentsTypes.length; i++){
		var componentsType = componentsTypes[i];
		world.getComponentManager().create(componentsTypes.getComponentType().getName(), componentsType);
	}
